  ...
  <div>
    ...
    >
      <template #phone="{ record }">
        {{ phoneFormat(record.supplierPhone) }}
      </template>
    </page-content>
    <page-modal
      ref="pageModalRef"
...
export default defineComponent({
  ...
  setup() {
    const pageName = ref('supplier')
    const pageTitle = ref('供应商')
    const requestUrl = InventoryAPI.Supplier
    ...
    const phoneFormat = (phoneNumber: string) => {
      return phoneNumber.replace(/(^\+86)(\d{3})(\d{4})(\d{4})/g, '$1 $2 $3 $4')
    }
