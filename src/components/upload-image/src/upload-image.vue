<template>
  <div class="clearfix">
    <a-upload
      ...
      @remove="handleRemove"
    >
      <div v-if="fileList.length < numberOfImage">
        <plus-outlined />
...
export default defineComponent({
  ...
  emits: ['uploadImage'],
  setup(props, { emit }) {
    const previewVisible = ref(false) // 预览图片是否显示
    const previewImage = ref('') // 预览图片的内容
    const previewTitle = ref('') // 预览图片的标题
